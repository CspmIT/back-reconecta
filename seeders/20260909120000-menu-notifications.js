...
module.exports = {
	async up(queryInterface) {
		...
		if (!parent) {
			console.log('No existe el menú Configuración: se omite el alta de Mis notificaciones.')
			return
		}

		const [[existing]] = await queryInterface.sequelize.query(
			"SELECT id FROM Menus WHERE link = 'config/notifications' LIMIT 1"
		)
		if (existing) {
			console.log('El menú Mis notificaciones ya existe: no se vuelve a insertar.')
			return
		}
		...
		// Habilitado para todos los perfiles: son las notificaciones del propio
		// usuario, no una configuracion del sistema.
