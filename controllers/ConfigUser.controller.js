...
const getColumnsUserTable = async (req, res) => {
	try {
		...
		const idTable = await getIdTable(req.db, table_name)
		const column = await getColumnsUser(req.db, idTable.id, id_user)
...
const getControlsRecloserNewData = async (req, res) => {
	try {
		...
		const controls = await getControlsRecloserNew(req.db, version)
...
const getControlsRecloserUser = async (req, res) => {
	try {
		...
		const controlsUser = await getControlsUserConfig(req.db, user, version)
		let result = []
		if (controlsUser.length) {
			...
			result = await getControlsRecloser(req.db, version)
		}
		return res.status(200).json(result)
	} catch (error) {
		if (error.errors) {
...
const saveControlsRecloser = async (req, res) => {
	try {
		...
		const result = await saveCrontrolsUser(req.db, controls)
		return res.status(200).json(result)
	} catch (error) {
		if (error.errors) {
			res.status(500).json(error.errors)
